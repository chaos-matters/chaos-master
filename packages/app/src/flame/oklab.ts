...
const cbrt = tgpu['~unstable'].fn(
  [f32],
  f32,
)/* wgsl */ `(x: f32) -> f32 { return pow(x, 1./3); }`

export const oklab2rgb = tgpu['~unstable'].fn([vec3f], vec3f)/* wgsl */ `
  (oklab: vec3f) -> vec3f {
...
export const rgb2oklab = tgpu['~unstable'].fn([vec3f], vec3f)/* wgsl */ `
  (rgb: vec3f) -> vec3f {
...
export const oklch2oklab = tgpu['~unstable'].fn([vec3f], vec3f)/* wgsl */ `
  (oklch: vec3f) -> vec3f {
    let L = oklch.x;
    let c = oklch.y;
    let h = oklch.z;
...
export const oklch2rgb = tgpu['~unstable'].fn([vec3f], vec3f)/* wgsl */ `
  (oklch: vec3f) -> vec3f {
    return oklab2rgb(oklch2oklab(oklch));
  }
`.$uses({ oklch2oklab, oklab2rgb })

const computeMaxSaturation = tgpu['~unstable'].fn([f32, f32], f32)/* wgsl */ `
  (a: f32, b: f32) -> f32 {
...
const findCusp = tgpu['~unstable'].fn([f32, f32], LC)/* wgsl */ `
  (a: f32, b: f32) -> LC {
