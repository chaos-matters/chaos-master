import { defineExample } from './util'

export const invCircleFrac2 = defineExample({
  metadata: { author: 'unknown' },
  renderSettings: {
    exposure: 0.25,
    skipIters: 3,
    drawMode: 'light',
    colorInitMode: 'colorInitZero',
    backgroundColor: [0, 0, 0],
    camera: {
      zoom: 0.8118146605137332,
      position: [0.29025036096572876, -0.05190691351890564],
    },
  },
  transforms: {
    ad6e4019_30fa_4e55_8806_5b15a5ad3de7: {
      probability: 0.2,
      preAffine: {
        a: 1,
        b: 0,
        c: 0,
        d: 0,
        e: 1,
        f: 0,
      },
      postAffine: { a: 1, b: 0, c: 0, d: 0, e: 1, f: 0 },
      color: { x: -0.10866179317235947, y: -0.13709218800067902 },
      variations: {
        f4bd8d90_690a_4378_9bdb_a9542c69ede2: {
          type: 'invCircle',
          weight: 1,
          params: { radius: 1, cx: -1.2, cy: 0, restricted: 1 },
        },
      },
    },
    '969939cb_f266_4b49_aff8_59f00f45040b': {
      probability: 0.2,
      preAffine: {
        a: 1,
        b: 0,
        c: 0,
        d: 0,
        e: 1,
        f: 0,
      },
      postAffine: { a: 1, b: 0, c: 0, d: 0, e: 1, f: 0 },
      color: { x: 0.06951913237571716, y: -0.041283342987298965 },
      variations: {
        f5a402db_3eee_46b4_b8be_b379d5513afd: {
          type: 'invCircle',
          weight: 1,
          params: { radius: 1, cx: 1.2, cy: 0, restricted: 1 },
        },
      },
    },
    '6ac1e8ed_9311_4d95_bc4a_8ac2a86447b8': {
      probability: 0.2,
      preAffine: {
        a: 1,
        b: 0,
        c: 0,
        d: 0,
        e: 1,
        f: 0,
      },
      postAffine: { a: 1, b: 0, c: 0, d: 0, e: 1, f: 0 },
      color: { x: -0.38098853826522827, y: 0.0391426607966423 },
      variations: {
        '709e0303_c73f_43d0_9406_e24fe4c888d9': {
          type: 'invCircle',
          weight: 1,
          params: { radius: 0.9, cx: 0, cy: 1, restricted: 1 },
        },
      },
    },
    '95e7a39b_6d0c_4e8b_bbfd_9500f99ed6e3': {
      probability: 0.2,
      preAffine: {
        a: 1,
        b: 0,
        c: 0,
        d: 0,
        e: 1,
        f: 0,
      },
      postAffine: { a: 1, b: 0, c: 0, d: 0, e: 1, f: 0 },
      color: { x: -0.12039130926132202, y: 0.09316394478082657 },
      variations: {
        '9500488b_9603_4e62_88b7_c5d078b2d85d': {
          type: 'invCircle',
          weight: 1,
          params: { radius: 0.9, cx: 0, cy: -1, restricted: 1 },
        },
      },
    },
    '8228ccf9_b2cf_4388_9c91_8d645087d5ee': {
      probability: 0.2,
      preAffine: {
        a: 1,
        b: 0,
        c: 0,
        d: 0,
        e: 1,
        f: 0,
      },
      postAffine: { a: 1, b: 0, c: 0, d: 0, e: 1, f: 0 },
      color: { x: 0.10011675208806992, y: 0.12697258591651917 },
      variations: {
        '83b9e42b_faec_4da0_9d80_b214fe82e8d1': {
          type: 'invCircle',
          weight: 1,
          params: { radius: 0.6, cx: 0, cy: 0, restricted: 1 },
        },
      },
    },
  },
})

export const invCircleFrac = defineExample({
  metadata: { author: 'unknown' },
  renderSettings: {
    exposure: 0.902,
    skipIters: 13,
    drawMode: 'light',
    colorInitMode: 'colorInitZero',
    backgroundColor: [0, 0, 0],
    camera: {
      zoom: 0.5296719699821935,
      position: [1.0272960662841797, 0.035816971212625504],
    },
  },
  transforms: {
    ad6e4019_30fa_4e55_8806_5b15a5ad3de7: {
      probability: 0.2,
      preAffine: {
        a: 1,
        b: 0,
        c: 0,
        d: 0,
        e: 1,
        f: 0,
      },
      postAffine: { a: 1, b: 0, c: 0, d: 0, e: 1, f: 0 },
      color: { x: -0.10866179317235947, y: -0.13709218800067902 },
      variations: {
        f4bd8d90_690a_4378_9bdb_a9542c69ede2: {
          type: 'invCircle',
          weight: 1,
          params: { radius: 0.3, cx: 1.4, cy: 0, restricted: 1 },
        },
      },
    },
    '969939cb_f266_4b49_aff8_59f00f45040b': {
      probability: 0.2,
      preAffine: {
        a: 1,
        b: 0,
        c: 0,
        d: 0,
        e: 1,
        f: 0,
      },
      postAffine: { a: 1, b: 0, c: 0, d: 0, e: 1, f: 0 },
      color: { x: 0.06951913237571716, y: -0.041283342987298965 },
      variations: {
        f5a402db_3eee_46b4_b8be_b379d5513afd: {
          type: 'invCircle',
          weight: 1,
          params: { radius: 1, cx: 0, cy: 0, restricted: 1 },
        },
      },
    },
    '6ac1e8ed_9311_4d95_bc4a_8ac2a86447b8': {
      probability: 0.2,
      preAffine: {
        a: 1,
        b: 0,
        c: 0,
        d: 0,
        e: 1,
        f: 0,
      },
      postAffine: { a: 1, b: 0, c: 0, d: 0, e: 1, f: 0 },
      color: { x: -0.38098853826522827, y: 0.0391426607966423 },
      variations: {
        '709e0303_c73f_43d0_9406_e24fe4c888d9': {
          type: 'invCircle',
          weight: 1,
          params: { radius: 0.3, cx: -1.4, cy: 1, restricted: 1 },
        },
      },
    },
    '95e7a39b_6d0c_4e8b_bbfd_9500f99ed6e3': {
      probability: 0.2,
      preAffine: {
        a: 1,
        b: 0,
        c: 0,
        d: 0,
        e: 1,
        f: 0,
      },
      postAffine: { a: 1, b: 0, c: 0, d: 0, e: 1, f: 0 },
      color: { x: -0.12039130926132202, y: 0.09316394478082657 },
      variations: {
        '9500488b_9603_4e62_88b7_c5d078b2d85d': {
          type: 'invCircle',
          weight: 1,
          params: { radius: 0.3, cx: 0, cy: 1.4, restricted: 1 },
        },
      },
    },
    '177ec5dd_a0c3_459c_bb74_428a1922d8eb': {
      probability: 1,
      color: { x: 0, y: 0 },
      preAffine: {
        a: 1,
        b: 0,
        c: 0,
        d: 0,
        e: 1,
        f: 0,
      },
      postAffine: { a: 1, b: 0, c: 0, d: 0, e: 1, f: 0 },
      variations: {
        bfc50fb7_c8fa_4817_8887_f99365e5e140: {
          type: 'invCircle',
          weight: 1,
          params: { radius: 1, cx: 0, cy: -1.4, restricted: 1 },
        },
      },
    },
  },
})
