...
> Cloudflare now supports "Full-stack Workers" where a single worker can serve static assets _and_ backend API routes. Since `chaos-master-fp` already uses `assets: { directory: "dist" }` in `wrangler.jsonc`, the cleanest approach is to add a small backend entrypoint (`src/worker/index.ts`) to the existing `app` package.
>
