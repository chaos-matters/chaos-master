...
| Camera                | pan, zoom, pinch, orbit, look, fly, 3D theta/phi/radius/fov/roll/target                                                                                                                                                                                                                                                                        | `flame.setRenderSetting` on `camera.*` / `camera3D.*`; agents use `camera.center/panTo/panBy/zoomTo/zoomBy/frame`, which resolve to the same anchors                |
...
| Arcade narration      | Agent narration lines during Teach and Cinema (no UI control; issued by the `arcade_narrate` tool)                                                                                                                                                                                                                                             | `lesson.note` (the text is the caption on replay)                                                                                                                   |
