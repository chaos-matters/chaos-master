...
| Audio             | audio **wiring** edits (mappings, sources)          | Not yet inspected in detail; treat as uncovered. See finding 1 below for the separate per-frame problem.                                                                                                                                                                                |
...
| Timeline          | every keyframe edit, track add/remove, duration/fps | **Structurally out of scope for v1.** The timeline has its own undo stack and is not part of the session format (`initialTracks` is designed but unimplemented). A recorded undo landing on a timeline edit is detected and reported as unreplayable rather than silently mis-replayed. |
